...
const cliLines = [
  { text: "$ kairo build ./spec/product-prd.md", cls: "font-bold text-brand-cyan", prefix: true, lineCls: "text-zinc-100" },
  { text: "[spec] Parsing requirements... 100%", cls: "text-zinc-500" },
  { text: "[spec] Architecture & data model generated", cls: "text-zinc-500" },
  { text: "[generator] Synthesizing Next.js 14 application", cls: "text-brand-purple-light" },
  ...
  { text: "[agent] Production edge build ready", cls: "font-medium text-brand-cyan" },
];
...
export function TerminalSection() {
  return (
    <section className="mx-auto max-w-6xl border-t border-white/[0.06] px-6 py-20">
      <FadeIn>
        <div className="rounded-lg border border-white/[0.08] bg-brand-surface p-6 font-mono-tech text-xs">
          ...
          <div className="space-y-1.5 text-[12px] leading-relaxed">
            {cliLines.map((line) => (
              <div key={line.text} className={line.lineCls ?? line.cls}>
                {line.prefix && <span className="font-bold text-brand-cyan">$ </span>}
